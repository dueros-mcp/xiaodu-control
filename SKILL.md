...
以 `mcporter` 作为主接入方式。除非用户明确证明小度智能屏 MCP 和小度 IoT MCP 共享同一个端点，否则把它们当作两个独立的 server 处理。不要猜测 MCP URL，必须使用小度平台控制台里给出的真实地址。
...
1. 先确认前提条件：
   ...
   - 如果要接 `xiaodu-iot`，用户已经准备好了 `ACCESS_TOKEN`，以及后续刷新的 `AppKey / SecretKey / refresh_token`。
...
- `scripts/refresh_iot_token.sh`
  - 用 `refresh_token` 刷新 `xiaodu-iot` 的 `ACCESS_TOKEN`，并自动回写 `mcporter` 配置。
- `scripts/install_iot_refresh_launchd.sh`
  - 在 macOS 上安装每日检查一次的自动刷新任务，只有临近过期时才真正刷新。
- `scripts/uninstall_iot_refresh_launchd.sh`
  - 移除 macOS 自动刷新任务。
...
- 当你要直接给用户一个可填写的安装模板时，读 [references/mcporter.template.json](references/mcporter.template.json) 和 [references/xiaodu-iot-oauth.template.json](references/xiaodu-iot-oauth.template.json)。
